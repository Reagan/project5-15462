...
namespace CMU462 {
    ...
    void MeshEdit::initializeStyle( void )
    {
        ...
        // added code - for selected items style
        combinedStyle.halfedgeColor = Color( 0.65, 0.65, 0.65 );
        combinedStyle.faceColor = Color( 0.4, 0.4, 0.80 );
        combinedStyle.edgeColor = Color( 0, 1, 0 );
        combinedStyle.vertexColor = Color( 0.0, 0.0, 0.00 );
        combinedStyle.strokeWidth = 4.0;
        combinedStyle.vertexRadius = 2.0;
        // end added code
    }
    ...
    void MeshEdit::key_event( char key )
    {
        switch( key ) {
            ...
            case 'y':
            case 'Y':
                setCommonCollectStatus();
                break ;
            case 'z':
            case 'Z':
                removeCommonCollectStatus() ;
                break ;

                // implementing catmull-clark subdivision
            case '0':
                addCatmullClarkSubdivisionLevels() ;
                break ;

            case '9':
                removeCatmullClarkSubdivisionLevels() ;
                break ;
    ...
    // - added methods
    void MeshEdit::setCommonCollectStatus( void ) {
        cout << "set common collect set!" << endl ;
        collecting_edges = true ;
        selectedMeshFeatures.clear() ;
    }

    void MeshEdit::removeCommonCollectStatus( void ) {
        cout << "set common collect reset!" << endl ;
        collecting_edges = false ;
        selectedMeshFeatures.clear() ;
    }

    inline void MeshEdit::addCatmullClarkSubdivisionLevels() {
        roundsCatmullClark = (roundsCatmullClark > 9) ?  roundsCatmullClark : roundsCatmullClark+1 ;
        HalfedgeMesh* mesh;
        ...
        HalfedgeMesh* newMesh = resampler.catmullClarkSubdivide( *mesh, roundsCatmullClark );
        renderMesh(*newMesh) ;
    ...
    inline void MeshEdit::removeCatmullClarkSubdivisionLevels() {
        roundsCatmullClark = (roundsCatmullClark <= 0) ? 0 : roundsCatmullClark-1;
        HalfedgeMesh* mesh;
        ...
        resampler.catmullClarkSubdivide( *mesh, roundsCatmullClark );
    ...
    void MeshEdit::scroll_event( float offset_x, float offset_y )
    {
        ...
        // Ensure appropriate bounds amounts.
    ...
    void MeshEdit::findMouseSelection(float x, float y)
    {
        ...
        /*
         * IMPORTANT NOTE: OpenGL coordinate system origin at bottom left of
    ...
    {
        cout << "selected feature is " << selectedFeature.element << endl ;
        selectedFeature = hoveredFeature;

        // add to set of selected features
        if (collecting_edges)
            selectedMeshFeatures.push_back(selectedFeature) ;
    }
    ...
    inline void MeshEdit::setElementStyle( HalfedgeElement* element )
    {
        ...
        // cout << "default style set..." << endl ;
        if( element == selectedFeature.element )
        {
            // cout << "select style set..." << endl ;
            style = &selectStyle;
        }
        else if( element == hoveredFeature.element )
        {
            // cout << "hover style called..." << endl ;
            style = &hoverStyle;
        } else if ( element == selectedFeature.element) {
            // cout << "selected style called..." << endl ;
            style = &combinedStyle ;
        }
    ...
    // - added element
    inline void MeshEdit::setMultipleElementStyle(HalfedgeElement* element)
    {
        // Set the draw style according to whether the
        // current mesh element is hovered or selected,
        // giving priority to selection.
        DrawStyle* style = &combinedStyle; // set the style for selected elements
    ...
    {
        // cout << endl << "draw edges called..." << endl ;
        for( EdgeIter e = mesh.edgesBegin(); e != mesh.edgesEnd(); e++ ) // iterate over edges
        {
            ...
            if (selectedMeshFeatures.size() > 0)
            {
                ...
                {
                    Edge* curr_e = ((MeshFeature) *iter).element->getEdge() ;
                    if (e->getEdge() == curr_e)
                    {
                        // this is one of the selected edges
                        setMultipleElementStyle(curr_e) ;
                        break ;
                    }
                }
            }
    ...
    void MeshEdit::drawVertices( void )
    {
        Vertex* v;
    ...
    void MeshEdit::drawHalfedges( HalfedgeMesh& mes )
    {
        // cout << "draw half edges called!" << endl ;
        Halfedge* h;
