...
class MeshEdit : public Renderer {
 ...
 private:
  ...
  int light_num;
    ...
    bool collecting_edges ;
    vector<MeshFeature> selectedMeshFeatures;
  ...
  // - added code
  DrawStyle combinedStyle; // style for elements that have been selected
  int roundsCatmullClark = 0 ;  // number of rounds for catmull clark subdivision
  // end added code
  ...
  inline void drawString(float x, float y, string str, size_t size, Color c);
    ...
    // -- additional methods
    void setCommonCollectStatus( void ) ;
    void removeCommonCollectStatus( void ) ;
    inline void setMultipleElementStyle(HalfedgeElement* element) ;
    inline void addCatmullClarkSubdivisionLevels() ;
    inline void removeCatmullClarkSubdivisionLevels() ;
